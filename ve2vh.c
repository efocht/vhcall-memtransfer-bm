...
#pragma omp parallel shared(h, par, sym_ve2vh, sym_alloc, sym_free, bandwidth, totalbw) private(i)
	{
#pragma omp for
		for (i = 0; i < par; i++) {
			...
			//printf("before fill addr=%p src=%p len=%d\n", srd.dst, srd.src, srd.size);
...
#pragma _NEC ivdep
			...
			if (vhcall_invoke(sym_ve2vh, &srd, sizeof(srd), 0, 0)) {
